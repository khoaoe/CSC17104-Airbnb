...
**Bài toán.** Dự đoán **giá thuê/đêm** (USD) của listing Airbnb tại NYC dựa trên thông tin listing (vị trí, loại phòng, lượt review, v.v.).

**Động lực & ứng dụng.**

* Chủ nhà định giá hợp lý; nền tảng gợi ý giá; nhà nghiên cứu phân tích yếu tố ảnh hưởng đến giá.

**Mục tiêu cụ thể.**

1. Xây dựng pipeline **EDA → Preprocessing → Modeling** tối giản với **NumPy**.
2. Tạo đặc trưng vị trí và phân loại gọn, giảm lệch đuôi bằng log-transform.
3. Huấn luyện mô hình tuyến tính (Ridge) + chọn siêu tham số bằng K-Fold CV.
...
* **Các feature chính (trích):**
  ...
  * Tín hiệu cầu: `number_of_reviews`, `last_review`, `reviews_per_month`
  * **Target:** `price` (USD/đêm)

**Đặc điểm dữ liệu:**

* CSV chuẩn **có quoting** (nhiều chuỗi chứa dấu phẩy trong `"..."`).
...
* Heatmap tương quan.
* Scatter **Pred vs True**, histogram residual.
...
* **Đặc trưng vị trí:** khoảng cách Haversine tới POI (Times Sq., Central Park, Wall St., …), geohash grid, tương tác `room_type × geo`.
